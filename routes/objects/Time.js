...
function findTime(){
    ...
    for(let i = 0;i<stopsArray.length; i++) {
        leavesArray[i] = [];
        let hours = Object.keys(timesArray[stopsArray[i]]);
            for (let j = 0; j < hours.length; j++) {
                let leaveTime = timesArray[stopsArray[i]][hours[j]];
...
function substractTime(leavesArray,lineNumber,direction){
    ...
    for(let i = 1; i<leavesArray.length; i++){
        ride[i]={};
        for(let j = 0; j<leavesArray[i].length-1; j++){
            let result = leavesArray[i][j]-leavesArray[i-1][j];
            if(result<0)
                result+=60;
            ride[i][j] = result;
        }
