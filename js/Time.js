    ...
    for(var i = 0;i<stopsArray.length; i++) {
        leavesArray[i] = [];
            ...
            for (var j = 0; j < hours.length; j++) {
               var leaveTime = timesArray[stopsArray[i]][hours[j]];
...
function substractTime(leavesArray,lineNumber,direction,stopsArray){
    var ride = [];
    ride[0] = [lineNumber,direction];
    for(var i = 1; i<leavesArray.length; i++){
        ride[i]={};
        for(var j = 0; j<leavesArray[i].length-1; j++){
            var result = leavesArray[i][j]-leavesArray[i-1][j];
            if(result<0)
                result+=60;
            ride[i][j] = result;
        }
        ...
        ride[i][j] = [directionName,stopsArray[i].coordnateX,stopsArray[i].coordnateY];
    }
    return ride;
}
function controller(busStopsResponse,linesResponse) {
    var allStopsArray=changeBusStopResponse(busStopsResponse.response);
    var leavesArray = findTime();
    var differences = substractTime(leavesArray, 47, leavesArray[leavesArray.length-1][leavesArray[0].length-1],allStopsArray);
    var line = prepare(differences);
    download(line);
}


var stopsArray=['felin spiessa','rondo przemyslowcow','rondo karszo-siedlewskiego','vetterow','felin europark',
    'doswiadczalna'];
...
function changeBusStopResponse(busStopResponse){
    var stopArray=[];
    for(var i = 0;i<busStopResponse.length;i++){
        stopArray.push({id:busStopResponse.id,coordinateX:busStopResponse.longitude, coordinateY:busStopResponse.latitude});
    }
    return stopArray;
}